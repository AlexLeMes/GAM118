...
int main()
{
	...
	while (window.isOpen())
	{
		...
		while (window.pollEvent(event))
		{
			...
			if (isSpriteHover(spriteHoverButton.getGlobalBounds(), Vector2f(event.mouseButton.x, event.mouseButton.y)) == true)
			{
				if (event.type == Event::MouseButtonReleased &&  event.mouseButton.button == Mouse::Left)
				{
					//RefreshTiles();
		...
		*/
		...
		spriteHoverButton.setPosition(0, 0);
		window.draw(spriteHoverButton);
...
/*
void RefreshTiles(int tileIndex)
{
	std::ofstream myfile;
	myfile.open("userData/tempData.txt");
	...
	for (row = 0; row < tileWidth; row++)
	{
		for (column = 0; column < tileHeight; column++)
		{
			fileData[tileIndex] = TileStyle[column][row];
			myfile << fileData[count];
			//count++;
		}
	}
...
void loadRefreshData()
{
	...
	std::ifstream myData("userData/tempData.txt");
